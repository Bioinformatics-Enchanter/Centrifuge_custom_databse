...
centrifuge-download -P 50 -o library -d "bacteria,viral" refseq > seqid2taxid.map
...
centrifuge-download -P 50 -o library -d "fungi" \
                    -t 746128,5059,5061,33178,5476,498019,42374,5478,\
                    4909,36911,5480,5482,5207,58839,5551,523103,1208 \
                    genbank >> seqid2taxid.map
# for example
# centrifuge-download -o library -d "fungi" -t 1280 refseq > seqid2taxid.map
######  error :Domain fungi has no genomes with specified filter ####
...
centrifuge-build -p 50 --conversion-table seqid2taxid.map3 \
